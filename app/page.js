...
import { FiArrowRight, FiShoppingCart, FiStar, FiPhoneCall, FiMail, FiMapPin, FiClock } from "react-icons/fi";
import { FaFacebookF, FaTwitter, FaInstagram, FaPinterest, FaYoutube, FaApple, FaGooglePlay, FaCcVisa, FaCcMastercard, FaCcAmex, FaCcDiscover } from "react-icons/fa";
...
const dealsOfTheDay = [
  { name: "Seeds of Change Organic Quinoa", price: "$12.85", oldPrice: "$13.8", img: "🥬" },
  { name: "Perdue Simply Smart Organics Gluten", price: "$12.85", oldPrice: "$13.8", img: "🍞" },
  { name: "Signature Wood-Fired Mushroom", price: "$12.85", oldPrice: "$13.8", img: "🥦" },
  { name: "Simply Lemonade with Raspberry", price: "$12.85", oldPrice: "$13.8", img: "🍋" },
];

const listProducts = [
  { name: "Nestle Original Coffee-Mate Coffee Creamer", price: "$28.85", oldPrice: "$32.8", img: "☕", rating: 4.0 },
  { name: "Organic Cage-Grade A Large Eggs", price: "$28.85", oldPrice: "$32.8", img: "🥚", rating: 4.0 },
  { name: "Seeds of Change Organic Quinoa", price: "$28.85", oldPrice: "$32.8", img: "🌾", rating: 4.0 },
];
    ...
    <div className="bg-gray-50 min-h-screen font-sans">
      ...
      <main className="container mx-auto px-4 py-8 space-y-12">
        ...
        <section>
          <div className="flex justify-between items-center mb-6">
            ...
            <a href="#" className="text-gray-500 text-sm hover:text-green-600 flex items-center gap-1">All Deals <FiArrowRight /></a>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {dealsOfTheDay.map((deal, idx) => (
              <div key={idx} className="relative rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow group">
                {/* Background Image / Placeholder */}
                <div className="h-64 bg-cover bg-center" style={{ backgroundImage: `url('https://images.unsplash.com/photo-1518843875459-f738682238a6?q=80&w=2542&auto=format&fit=crop')` }}>
                  <div className="absolute inset-0 bg-black/10 group-hover:bg-black/20 transition-colors"></div>
                </div>
                {/* Content Overlay/Card */}
                <div className="absolute bottom-4 left-4 right-4 bg-white rounded-xl p-4 shadow-lg">
                  <h4 className="font-bold text-gray-800 text-sm mb-2 truncate">{deal.name}</h4>
                  <div className="flex gap-2 mb-3 justify-center">
                    <div className="bg-yellow-100 rounded p-1 text-center min-w-[32px]"><span className="block font-bold text-green-600 text-xs">02</span><span className="text-[10px] text-gray-500">Days</span></div>
                    <div className="bg-yellow-100 rounded p-1 text-center min-w-[32px]"><span className="block font-bold text-green-600 text-xs">22</span><span className="text-[10px] text-gray-500">Hrs</span></div>
                    <div className="bg-yellow-100 rounded p-1 text-center min-w-[32px]"><span className="block font-bold text-green-600 text-xs">18</span><span className="text-[10px] text-gray-500">Min</span></div>
                    <div className="bg-yellow-100 rounded p-1 text-center min-w-[32px]"><span className="block font-bold text-green-600 text-xs">04</span><span className="text-[10px] text-gray-500">Sec</span></div>
                  </div>
                  <div className="flex justify-between items-center bg-white">
                    <div>
                      <span className="text-green-600 font-bold">{deal.price}</span>
                      <span className="text-gray-400 text-xs line-through ml-2">{deal.oldPrice}</span>
                    </div>
                    <button className="bg-green-100 text-green-600 hover:bg-green-600 hover:text-white px-3 py-1.5 rounded-md transition-colors font-bold text-xs flex items-center gap-1">Add <FiShoppingCart /></button>
                  </div>
                </div>
                {/* Center Image */}
                <div className="absolute top-10 left-1/2 -translate-x-1/2 text-6xl drop-shadow-lg">{deal.img}</div>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          <div>
            <h3 className="text-xl font-bold text-gray-800 mb-6 border-b pb-2 relative after:content-[''] after:absolute after:bottom-0 after:left-0 after:w-16 after:h-0.5 after:bg-green-400">Top Selling</h3>
            <div className="space-y-6">
              {listProducts.map((prod, i) => (
                <div key={i} className="flex gap-4 group cursor-pointer">
                  <div className="w-20 h-20 bg-gray-100 rounded-lg flex items-center justify-center text-3xl">{prod.img}</div>
                  <div>
                    <h5 className="font-bold text-gray-700 text-sm mb-1 group-hover:text-green-600 transition-colors line-clamp-2">{prod.name}</h5>
                    <div className="flex text-yellow-400 text-[10px] mb-1">
                      {[...Array(5)].map((_, j) => <FiStar key={j} className={j < Math.floor(prod.rating) ? "fill-current" : "text-gray-300"} />)}
                    </div>
                    <div>
                      <span className="text-green-600 font-bold text-sm">{prod.price}</span>
                      <span className="text-gray-400 text-xs line-through ml-2">{prod.oldPrice}</span>
                    </div>
                  </div>
                </div>
          ...
          <div>
            <h3 className="text-xl font-bold text-gray-800 mb-6 border-b pb-2 relative after:content-[''] after:absolute after:bottom-0 after:left-0 after:w-16 after:h-0.5 after:bg-green-400">Trending Products</h3>
            <div className="space-y-6">
              {listProducts.map((prod, i) => (
                <div key={i} className="flex gap-4 group cursor-pointer">
                  <div className="w-20 h-20 bg-gray-100 rounded-lg flex items-center justify-center text-3xl">{prod.img}</div>
          ...
          <div>
            <h3 className="text-xl font-bold text-gray-800 mb-6 border-b pb-2 relative after:content-[''] after:absolute after:bottom-0 after:left-0 after:w-16 after:h-0.5 after:bg-green-400">Recently added</h3>
            <div className="space-y-6">
              {listProducts.map((prod, i) => (
                <div key={i} className="flex gap-4 group cursor-pointer">
                  <div className="w-20 h-20 bg-gray-100 rounded-lg flex items-center justify-center text-3xl">{prod.img}</div>
          ...
          {/* Column 4: Top Rated */}
          <div>
            <h3 className="text-xl font-bold text-gray-800 mb-6 border-b pb-2 relative after:content-[''] after:absolute after:bottom-0 after:left-0 after:w-16 after:h-0.5 after:bg-green-400">Top Rated</h3>
            <div className="space-y-6">
              {listProducts.map((prod, i) => (
                <div key={i} className="flex gap-4 group cursor-pointer">
                  <div className="w-20 h-20 bg-gray-100 rounded-lg flex items-center justify-center text-3xl">{prod.img}</div>
      ...
      <footer className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-8 text-sm text-gray-500 pb-8 border-b">
          ...
          <div className="col-span-1 lg:col-span-1">
            <div className="flex items-center gap-2 mb-4">
              ...
              <div>
                <span className="text-2xl font-bold text-green-600 leading-none block">Nest</span>
                <span className="text-[10px] text-gray-400 block">Mart & Grocery</span>
              </div>
            </div>
            <p className="mb-4">Awesome grocery store website template</p>
            <ul className="space-y-2">
              <li className="flex gap-2 items-start"><FiMapPin className="text-green-600 mt-1 shrink-0" /> <span><strong>Address:</strong> 5171 W Campbell Ave, Utah 53127 United States</span></li>
              <li className="flex gap-2 items-center"><FiPhoneCall className="text-green-600 shrink-0" /> <span><strong>Call Us:</strong> (+91) - 540-025-124553</span></li>
              <li className="flex gap-2 items-center"><FiMail className="text-green-600 shrink-0" /> <span><strong>Email:</strong> sale@Nest.com</span></li>
              <li className="flex gap-2 items-center"><FiClock className="text-green-600 shrink-0" /> <span><strong>Hours:</strong> 10:00 - 18:00, Mon - Sat</span></li>
            </ul>
          </div>

          {/* Links Columns */}
          <div><h4 className="font-bold text-lg text-gray-800 mb-4">Company</h4><ul className="space-y-2 cursor-pointer"><li>About Us</li><li>Delivery Information</li><li>Privacy Policy</li><li>Terms & Conditions</li><li>Contact Us</li><li>Support Center</li><li>Careers</li></ul></div>
          <div><h4 className="font-bold text-lg text-gray-800 mb-4">Account</h4><ul className="space-y-2 cursor-pointer"><li>Sign In</li><li>View Cart</li><li>My Wishlist</li><li>Track My Order</li><li>Help Ticket</li><li>Shipping Details</li><li>Compare products</li></ul></div>
          <div><h4 className="font-bold text-lg text-gray-800 mb-4">Corporate</h4><ul className="space-y-2 cursor-pointer"><li>Become a Vendor</li><li>Affiliate Program</li><li>Farm Business</li><li>Farm Careers</li><li>Our Suppliers</li><li>Accessibility</li><li>Promotions</li></ul></div>
          <div><h4 className="font-bold text-lg text-gray-800 mb-4">Popular</h4><ul className="space-y-2 cursor-pointer"><li>Milk & Flavoured Milk</li><li>Butter and Margarine</li><li>Eggs Substitutes</li><li>Marmalades</li><li>Sour Cream and Dips</li><li>Tea & Kombucha</li><li>Cheese</li></ul></div>

          {/* Install App */}
          <div>
            <h4 className="font-bold text-lg text-gray-800 mb-4">Install App</h4>
            <p className="mb-4">From App Store or Google Play</p>
            <div className="flex gap-2 mb-6">
              <button className="bg-black text-white px-3 py-2 rounded-lg flex items-center gap-2 text-xs hover:bg-gray-800 transition"><FaApple className="text-xl" /> <div><div className="text-[8px]">Download on the</div><div className="font-bold">App Store</div></div></button>
              <button className="bg-black text-white px-3 py-2 rounded-lg flex items-center gap-2 text-xs hover:bg-gray-800 transition"><FaGooglePlay className="text-xl" /> <div><div className="text-[8px]">GET IT ON</div><div className="font-bold">Google Play</div></div></button>
            </div>
        ...
        <div className="pt-8 flex flex-col md:flex-row justify-between items-center gap-4 text-sm text-gray-500">
          <div>© 2024, <strong className="text-green-600">Nest</strong> - HTML Ecommerce Template <br /> All rights reserved</div>
          <div className="flex gap-8 items-center">
            <div className="flex items-center gap-3">
              <FiPhoneCall className="text-3xl text-gray-500" />
              <div><div className="text-green-600 text-xl font-bold">1900 - 6666</div><div className="text-[10px]">Working 8:00 - 22:00</div></div>
            </div>
            <div className="flex items-center gap-3">
              <FiPhoneCall className="text-3xl text-gray-500" />
              <div><div className="text-green-600 text-xl font-bold">1900 - 8888</div><div className="text-[10px]">24/7 Support Center</div></div>
            </div>
          </div>
          <div className="flex gap-2">
            <span className="font-bold text-gray-800 mr-2">Follow Us</span>
            <div className="bg-green-600 text-white p-1.5 rounded-full cursor-pointer hover:bg-green-700 transition"><FaFacebookF /></div>
            <div className="bg-green-600 text-white p-1.5 rounded-full cursor-pointer hover:bg-green-700 transition"><FaTwitter /></div>
            <div className="bg-green-600 text-white p-1.5 rounded-full cursor-pointer hover:bg-green-700 transition"><FaInstagram /></div>
            <div className="bg-green-600 text-white p-1.5 rounded-full cursor-pointer hover:bg-green-700 transition"><FaPinterest /></div>
            <div className="bg-green-600 text-white p-1.5 rounded-full cursor-pointer hover:bg-green-700 transition"><FaYoutube /></div>
          </div>
        </div>
      </footer>
    </div>
