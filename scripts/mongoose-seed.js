...
const seedDatabase = async () => {
  ...
  },
  {
    ...
    if (productsToSeed.length === 0 && categoriesToSeed.length === 0) {
      console.warn(
        "⚠️ No data found to seed. Please paste your arrays into the script.",
      );
      process.exit(0);
    }

    // --- SEED CATEGORIES ---
    console.log(`📂 Migrating ${categoriesToSeed.length} categories...`);
    for (const cat of categoriesToSeed) {
      console.log(`  Uploading Category: ${cat.name}`);
      const uploadRes = await cloudinary.v2.uploader.upload(cat.img, {
    ...
    // --- SEED PRODUCTS ---
    console.log(`🛒 Migrating ${productsToSeed.length} products...`);
    for (const prod of productsToSeed) {
      try {
        console.log(`  Uploading Product: ${prod.name}`);
        const uploadRes = await cloudinary.v2.uploader.upload(prod.img, {
    ...
    console.log(
      "\n✅ Seeding complete! Database is now live with Cloudinary links.",
    );
  } catch (error) {
